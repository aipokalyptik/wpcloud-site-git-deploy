...
This file is the behavioral contract for the clean-room Go implementation. The
Bash implementation under `spec/` is the executable oracle, but these invariants
name the correctness properties that must survive the rewrite.

## Public Symlinks And Docroot Containment

- Every HTTP-visible public symlink created by the tool must use a relative
  target.
- Every public symlink target must resolve under the real docroot.
- Public symlink targets must never point into `$HOME`; on WP Cloud, `$HOME` is
  available over SSH but is not mounted for HTTP requests.
- Deploy-time assertion may be scoped to final owned claims, but `doctor` must
  retain a full-docroot public symlink audit.
...
- A successful deploy writes release metadata only after the engine promotion has
  completed.
...
- Existing public paths are reclaimed atomically by exchanging the new symlink
  with the old path.
- The Go implementation must use `renameat2(RENAME_EXCHANGE)` in process.
...
- Claim reconciliation must merge:
  - claims derived from the previous release tree;
  - materialized public symlinks still present in the docroot;
  - claims derived from the new release tree.
...
- Newline-containing public paths are unsupported and must fail before promotion.
...
- Rollback selects metadata-backed releases and may target an explicit release id.
