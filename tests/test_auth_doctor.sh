...
[[ "$("$cli" --version)" == "1.1.4" ]] || fail "--version should report current release line"
...
config_dir="$home_dir/.wpcloud-site-git-deploy/deployments/site"
[[ -f "$key_path" ]] || fail "auth should create private key"
[[ -f "$key_path.pub" ]] || fail "auth should create public key"
[[ -d "$config_dir" ]] || fail "init should create deployment config directory"
[[ ! -e "$home_dir/.wpcloud-site-git-deploy/deployments/site.env" ]] || fail "init should not create shell env deployment config"
[[ "$(cat "$config_dir/cfg-ssh_key_path")" == "$key_path" ]] || fail "auth should store ssh_key_path in cfg-ssh_key_path"
[[ "$(cat "$config_dir/cfg-repo_url")" == "git@github.com:example/private-site.git" ]] || fail "auth should store converted repo_url in cfg-repo_url"
...
[[ ! -e "$config_dir/cfg-ssh_key_path" ]] || fail "auth --remove should delete cfg-ssh_key_path"
...
[[ ! -e "$config_dir/cfg-ssh_key_path" ]] || fail "auth --remove --purge-key should delete cfg-ssh_key_path"
...
[[ "$(cat "$config_dir/cfg-ssh_key_path")" == "$external_key" ]] || fail "auth --use-key should store external key path"
...
[[ "$(cat "$config_dir/cfg-ssh_key_path")" == "$key_path" ]] || fail "auth --import-key should store managed key path"
...
https_config_dir="$https_home/.wpcloud-site-git-deploy/deployments/https-site"
https_key_path="$https_home/.wpcloud-site-git-deploy/keys/https-site_ed25519"
[[ "$(cat "$https_config_dir/cfg-repo_url")" == "git@git.example.com:team/site.git" ]] || fail "auth should convert generic HTTPS repo URL"
[[ "$(cat "$https_config_dir/cfg-ssh_key_path")" == "$https_key_path" ]] || fail "auth should store generated generic HTTPS key path"
